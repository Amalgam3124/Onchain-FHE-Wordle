...
import FHEWordleABI from '../contracts/FHEWordle.json';
import { useRelayerFHE } from '../hooks/useRelayerFHE';
...
const WordleGame = () => {
  ...
  const { relayerReady, encryptGuess, userDecryptEuint8Array } = useRelayerFHE(walletProvider);
  ...
  useEffect(() => {
    const initContract = async () => {
      try {
        if (isConnected && walletProvider && chainId === 11155111) {
  ...
  const switchToSepoliaW3m = async () => {
    try {
      ...
      console.warn('Failed to switch network to Sepolia:', e);
      setError('Failed to switch network to Sepolia');
    }
  };
  ...
  const submitGuess = async () => {
    ...
    // If on-chain is initialized, allow direct guesses too
    ...
    try {
      ...
      if (!relayerReady) {
        setError('Relayer SDK not ready, cannot submit encrypted guess');
        setLoading(false);
        return;
      }
      if (!contract) {
      ...
      const enc = await encryptGuess(currentGuess, CONTRACT_ADDRESS, address!);
      if (!enc) {
        setError('Encryption failed');
        setLoading(false);
        return;
      }
      if (enc.encryptedGuess.length !== 5 || enc.proofs.length !== 5) {
        setError(
          `Abnormal encryption output length: encryptedGuess=${enc.encryptedGuess.length}, proofs=${enc.proofs.length}, expected 5`,
        );
        setLoading(false);
        return;
      }
  
      // Add debug logs: check hex format and length
      try {
        const lensEG = enc.encryptedGuess.map((x) => (typeof x === 'string' ? x.length : -1));
        const lensPF = enc.proofs.map((x) => (typeof x === 'string' ? x.length : -1));
        console.debug(
          '[submitGuess] encryptedGuess len:',
          lensEG,
          'proofs len:',
          lensPF,
          'eg[0]:',
          enc.encryptedGuess[0],
          'pf[0]:',
          enc.proofs[0],
        );
      } catch {}
  
      // Pass proofs as byte arrays (Uint8Array) to avoid hex encoding differences
      const proofsBytes = enc.proofs.map((p) => {
        try {
          return ethers.getBytes(p as any);
        } catch (e) {
          console.warn('Failed to convert proof to bytes, keeping original value:', e);
          return p as any;
        }
      });
  
      // Use staticCall (ethers v6) to get returned encrypted result, then send transaction
      let returned: any;
      try {
        returned = await (contract as any).getFunction('submitGuess').staticCall(enc.encryptedGuess, proofsBytes);
      } catch (preErr: any) {
        console.error('Pre-check before submitting guess failed:', preErr);
        // Show full revert data and selector for debugging
        try {
          const data: string | Uint8Array | undefined = preErr?.data;
          if (data) {
            const hex = typeof data === 'string' ? data : ethers.hexlify(data as any);
            console.error('revert data (hex):', hex);
            // Parse selector
            ...
            if (selector === '0x08c379a0') {
              try {
                ...
                console.error('Standard Error(string):', parsed?.args?.[0]);
                setError('On-chain submit pre-check failed: ' + String(parsed?.args?.[0] || 'Unknown error'));
              } catch (e) {
                setError(
                  'On-chain submit pre-check failed: execution reverted (Error(string))\nselector=0x08c379a0\nhex=' +
                    hex,
                );
              }
            } else {
              // Show full revert data and selector
              setError(
                'On-chain submit pre-check failed: execution reverted (unknown custom error)\nselector=' +
                  selector +
                  '\nhex=' +
                  hex,
              );
            ...
            setError(
              'On-chain submit pre-check failed: ' +
                (preErr?.reason || preErr?.shortMessage || preErr?.message || 'Unknown error'),
            );
          }
        } catch {
          setError(
      ...
      const tx = await (contract as any).submitGuess(enc.encryptedGuess, proofsBytes);
      await tx.wait();
  
      const res = await userDecryptEuint8Array(returned, CONTRACT_ADDRESS, address!);
      if (!res) {
        setError('Decryption failed: please check that Relayer SDK is initialized correctly');
        setLoading(false);
        return;
      }
      ...
      if (won && nftContract) {
        try {
          ...
          console.error('Failed to mint daily solved NFT:', e);
        }
      }
    } catch (err: any) {
      console.error('Failed to submit guess:', err);
      setError('Failed to submit guess: ' + (err?.message || String(err)));
    } finally {
      setLoading(false);
      setLoadingReason(null);
    }
    ...
    <div className="wordle-root min-h-screen text-white">
      <div className="wordle-card">
        ...
        <div className="mb-4">
          ...
          {isConnected && chainId === 11155111 && contract && (
            <button
              onClick={syncState}
              disabled={loading}
