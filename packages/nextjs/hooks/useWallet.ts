...
import { sepoliaNetwork, getNetworkSwitchParams, isSupportedNetwork } from '../utils/networks';
...
export const useWallet = () => {
  ...
  // Target chain ID: prefer environment variable, otherwise fallback to Sepolia
  const targetChainId =
  ...
  // Switch to target network (Sepolia by default)
  const switchToSepolia = async () => {
    ...
    try {
      ...
      if (switchError.code === 4902) {
        try {
          const networkParams = getNetworkSwitchParams({ ...sepoliaNetwork, id: targetChainId });
          if (networkParams) {
            await provider.request({
  ...
  const connectWalletConnect = async () => {
    try {
      ...
      const defaultRpc = process.env.NEXT_PUBLIC_RPC_URL || 'https://rpc.sepolia.org';
      const wc = await EthereumProvider.init({
        ...
        showQrModal: true,
        metadata: {
          name: 'Zama FHE Wordle',
          description: 'Privacy-preserving Wordle built on FHEVM',
          url: typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000',
          icons: ['https://raw.githubusercontent.com/zama-ai/fhevm/main/logo.png'],
        },
      });
      await wc.enable();
